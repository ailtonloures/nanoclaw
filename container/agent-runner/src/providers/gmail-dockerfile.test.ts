 ...
 * `container/cli-tools.json` (consumed by `install-cli-tools.sh`, which batches every
 * entry into one pinned `pnpm install -g` call) — it is not importable or typed from
 * this tree, so the build leg can't catch its removal and there's no runtime seam to
 * behavior-test. This asserts the manifest still carries the pinned entry and the
 * zod-to-json-schema workaround. Drop either and this goes red, signalling the agent
 * would boot without the `gmail-mcp` binary on PATH (or hit ERR_PACKAGE_PATH_NOT_EXPORTED).
 */
import fs from 'fs';
